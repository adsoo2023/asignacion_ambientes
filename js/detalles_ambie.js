...
function mostrar_detalles(
  ...
  $(".num_ambiente").text(num_ambiente);
  // MODIFIQUE
...
function mostrarProgramas(event) {
  var self = this;
  if ($(self)) {
    $(self).addClass("animated-input-cargando");
  }
  var fichaP = document.querySelector("#form-da-numF");
  ...
  var valor_fichajs = fichaP.value;
  ...
  $.ajax({
    ...
    success: function (programa) {
      ...
      if (!programa.estado) {
        ...
        const ficha = programa.ficha;
        const nomPrograma = programa.nombreprograma;
        $("#form-da-nomF").val(nomPrograma);
        $("#form-da-numF").val(ficha);
      }
    },
    error: function (xhr, status, error) {
      console.error(error);
...
function mostrar_dispo_reser(event) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      res.forEach(element => {
        const lista_dispo_reser = $("<li class='lista-reser-dispo'><b>Fechas libres: </b> De <b>"+ element.fecha_inicio_reporte +" </b> a <b>"+ element.fecha_fin_reporte +"</b>, Motivo: <i></i>"+ element.motivo +".</li>")
