...
if ($res) {
    while ($fila = mysqli_fetch_assoc($res)) {
        ...
        if ($_SESSION['rol'] !== 1) {
            $usua = $fila["idusuario"];
            if ($usua == $id_usuario) {
                if ($fecha_inicio_times == $fechaI_times) {
                    $estados = [
...
if ($stmt->execute()) {
    ...
    if ($datos_reserva["numero_ficha"] == 1) {
        $contenido_correo = '
        <h2 style="color: #ccc; border-bottom:1px solid; width: 100%; padding: 8px;">Informacion del ambiente a reservar</h2><br>
        <h3 style="color: #ccc;">Nombre del ambiente: </h3><p>' . $datos_reserva["nombre_ambiente"] . '</p>
        <h3 style="color: #ccc;">Numero del ambiente: </h3><p>' . $datos_reserva["numero_ambiente"] . '</p><br><br>
        <h2 style="color: #ccc; border-bottom:1px solid; width: 100%; padding: 8px;">Información de la persona que realizo la reserva</h2><br>
        <h3 style="color: #ccc;">Nombre: </h3><p>' . $datos_reserva["nombre_usuario"] . ' ' . $datos_reserva["apellido"] . '</p>
        <h3 style="color: #ccc;">Numero de documento: </h3><p>' . $datos_reserva["documento"] . '</p>
        <h3 style="color: #ccc;">Correo: </h3><p>' . $datos_reserva["correo"] . '</p>
        <h3 style="color: #ccc;">Teléfono: </h3><p>' . $datos_reserva["telefono"] . '</p>
        <h3 style="color: #ccc;">Tipo de rol: </h3><p>' . $datos_reserva["nombre_rol"] . '</p><br><br>
        <h2 style="color: #ccc; border-bottom:1px solid; width: 100%; padding: 8px;">Informacion de la reserva</h2><br>
        <h3 style="color: #ccc;">Tipo de reserva: </h3><p>Reunion corta</p>
        <h3 style="color: #ccc;">Motivo: </h3><p>' . $datos_reserva["motivo"] . '</p>
        <h3 style="color: #ccc;">Fecha: </h3><p>Fecha reservada del <b>' . $fecha_inici . '</b> hasta el <b>' . $fecha_fi . '</b></p>
        <h3 style="color: #ccc;">Horas: </h3><p>De <b>' . $hora_inicio . '</b> hasta <b>' . $hora_fin . '</b></p><br>
        ...
        <h3 style="color: #ccc;">Tipo de rol: </h3><p>' . $datos_reserva["nombre_rol"] . '</p><br><br>
        <h2 style="color: #ccc; border-bottom:1px solid; width: 100%; padding: 8px;">Informacion de la reserva</h2><br>
        <h3 style="color: #ccc;">Tipo de reserva: </h3><p>Formación</p>
        <h3 style="color: #ccc;">Motivo: </h3><p>' . $datos_reserva["motivo"] . '</p>
        <h3 style="color: #ccc;">Ficha: </h3><p>' . $tipReserva . '</p>
        <h3 style="color: #ccc;">Formación: </h3><p>' . $datos_reserva["formacion"] . '</p>
        <h3 style="color: #ccc;">Jornada: </h3><p>' . $datos_reserva["jornada"] . '</p>
        <h3 style="color: #ccc;">Fecha: </h3><p>Fecha reservada del <b>' . $fecha_inici . '</b> hasta el <b>' . $fecha_fi . '</b></p>
        <h3 style="color: #ccc;">Horas: </h3><p>De <b>' . $hora_inicio . '</b> hasta <b>' . $hora_fin . '</b></p><br>
        <div style="margin: auto; display: flex; align-items: center; justify-content: space-between; width:100%;">
        <a href="https://adso24.com/asignar/admin.php" style="background-color: #66bb6a; color: white; padding: 10px 15px; text-align: center; border-radius: 4px; cursor: pointer;">Aceptar reserva</a>
        <a href="https://adso24.com/asignar/admin.php" style="background-color: #ff5a5a;   margin-left: 50px; color: white; padding: 10px 15px; text-align: center; border-radius: 4px; cursor: pointer;">Rechazar reserva</a>
        </div>
        ';
    }
